...
public class StreamingWordCount {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // 这里设置一秒 Durations.seconds(1)
        JavaStreamingContext jssc = new JavaStreamingContext(conf, Durations.seconds(1));

        // 首先创建输入DStream。代表了一个从数据源（比如Kafka、socket）来的持续不断的实时数据流
        // 调用JavaStreamingContext的socketTextStream() 方法，可以创建一个 数据源 为socket 网络端口的数据流
        ...
        JavaDStream<String> words = lines.flatMap(new FlatMapFunction<String, String>() {
            private static final long serialVersionUID = -6630941635006703538L;
            @Override
        ...
        JavaPairDStream<String, Integer> pairs = words.mapToPair(new PairFunction<String, String, Integer>() {
            private static final long serialVersionUID = -2632916155512843178L;
        ...
        JavaPairDStream<String, Integer> wordCounts = pairs.reduceByKey(new Function2<Integer, Integer, Integer>() {
            private static final long serialVersionUID = -8342326768260844472L;
            @Override
